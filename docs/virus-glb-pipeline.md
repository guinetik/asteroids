...
3. **Normals** — **`VIRUS_STRIP_NORMALS` defaults off**. The hologram shader reads vertex normals for fresnel; stripping makes the mesh render black unless you change materials. Opt in with `VIRUS_STRIP_NORMALS=true` only after confirming the runtime shader.
...
| `VIRUS_STRIP_NORMALS` | `false` | Strip NORMAL attributes before encode (`true` only if shaders do not rely on normals) |
