...
describe('TerminalModel', () => {
  it('renders as a tall matte kiosk with a large emissive display', () => {
    ...
    expect(screenGeometry.parameters.height).toBeGreaterThan(4)
    expect(screenGeometry.parameters.width).toBeGreaterThan(3.5)
    ...
    expect(base.position.y).toBeLessThan(0)
  ...
  it('exposes a world-space AABB collider around the kiosk footprint', () => {
    ...
    expect(min.y).toBe(0)
    expect(max.y).toBeGreaterThan(20)
