...
describe('ASTEROID_CATALOG', () => {
  ...
  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" has all required string fields', (id) => {
  ...
  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" composition sums to 100', (id) => {
    const asteroid = ASTEROID_CATALOG.find((a) => a.id === id)!
    const sum = asteroid.composition.reduce((acc, m) => acc + m.percentage, 0)
    expect(sum).toBe(100)
  })

  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" has valid shape ranges', (id) => {
  ...
  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" has valid surface ranges', (id) => {
  ...
  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" has valid visual ranges', (id) => {
  ...
  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" has valid physical ranges', (id) => {
  ...
  it.each(EXPECTED_ASTEROID_IDS)('asteroid "%s" minerals all exist in MINERAL_VISUALS', (id) => {
    const asteroid = ASTEROID_CATALOG.find((a) => a.id === id)!
