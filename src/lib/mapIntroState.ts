...
export const MAP_INTRO_CAPTION_PHOBOS =
  'A DISCOVERY ON THE MARTIAN MOON PHOBOS UNLOCKED RELATIVISTIC ACCELERATION AT OUR FINGERTIPS: THE NEUTRON THRUSTER.'

/** Caption: Viroid reveal on Phobos. */
export const MAP_INTRO_CAPTION_VIROIDS =
  'BUT THE MOON WAS ALSO HIDING SOMETHING IN STASIS: SILICATE CREATURES FROM INTERSTELLAR SPACE. TERRITORIAL. LETHAL. WE CALL THEM VIROIDS.'
...
export const MAP_INTRO_CAPTION_SATURN =
  'AT SATURN, THE ELITE OF HUMANITY LIVE SUPER HUMAN LIFESPANS. BELOW THE FREEZING POINT OF VIROIDS, OUT HERE THEY CAN ONLY EXIST IN STASIS.'

/**
 * Caption: retired lander operator receives shuttle.
 * Contains the `{name}` token, replaced with the player's display name at emit time
 * (see `MapViewController.emitIntroUiState`).
 */
export const MAP_INTRO_CAPTION_RETIRED_OPERATOR =
  'BACK ON EARTH, A RETIRED LANDER OPERATOR NAMED {name} JUST RECEIVED A REFURBISHED SPACE SHUTTLE FROM THE SPACE PROGRAM. HE IS READY TO TAKE OFF AGAIN.'
