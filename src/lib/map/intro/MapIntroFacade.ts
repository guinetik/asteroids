...
import { INTRO_ZOOM_STEPS, type IntroCinematicStep, type MapIntroState } from '@/lib/mapIntroState'
import {
  EARTH_MOON_INDEX,
  PHOBOS_MOON_INDEX,
...
export class MapIntroFacade {
  ...
  tick(params: {
    ...
    if (mapIntro.phase === 'cinematic_zoom') {
      ...
      this.tickIntroStep(
        ...
        vehicleCamera,
        shuttleController,
        params.findPlanetControllerById,
      )
      return
    }
  ...
  private tickIntroZoomPhobos(
    ...
    const marsCenter = this.tempMarsWorldPos.set(
      mars.getWorldX(),
      mars.getWorldY(),
      mars.getWorldZ(),
    )
  ...
  private tickIntroZoomSaturn(
    ...
    const jupiterCenter = this.tempFromLook.set(
      jupiter.getWorldX(),
      jupiter.getWorldY(),
      jupiter.getWorldZ(),
    )
    const saturnCenter = this.tempToLook.set(
      saturn.getWorldX(),
      saturn.getWorldY(),
      saturn.getWorldZ(),
    )
  ...
  private tickIntroZoomShuttle(
    ...
    const fromLook = saturn
      ? this.tempFromLook.set(saturn.getWorldX(), saturn.getWorldY(), saturn.getWorldZ())
